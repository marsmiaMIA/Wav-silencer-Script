...
- Windows
- [FFmpeg](https://ffmpeg.org/download.html#build-windows) installed and added to your system PATH
...
1. **Back up your `.wav` files** — the script overwrites originals, so keep a copy somewhere outside the target folder
2. Drop `reduce_wav_volume.bat` anywhere you like
3. Either:
...
4. The script will process all `.wav` files in the folder and any subfolders
...
This script **overwrites** the original `.wav` files. If you want to keep the originals, make sure to copy them to a folder that is **not** in the same directory you're running the script on before you start.
or a quick tutorial — it only takes a couple of minutes.
---
How to use
